...
namespace PickMeUp.Api.Account.AccountPreferences.SocialPreferences
{
    ...
    [Authorize]
    public sealed class SocialPreferencesController : ControllerBase
    {
        private readonly IAccountPreferencesRepository _repo;
        private readonly ICurrentUser _currentUser;

        public SocialPreferencesController(IAccountPreferencesRepository repo, ICurrentUser currentUser)
        {
            _repo = repo;
        ...
        {
            var newVersion = await _repo.UpdateSocialPreferencesAsync(_currentUser.AccountId, dto.ToSettings(), dto.Version);
            var doc = await _repo.GetOrCreateAsync(_currentUser.AccountId);
