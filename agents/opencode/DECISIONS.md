...
- **v0.0.38:** Chapter 26 documented via audit workflow — Floor 10 opens as highest-difficulty **Defend** mission (three-part warning): objective "Stop the fall of the city", fail condition = 'Twin Goddesses' statue destroyed → all heroes die, no exceptions; enemy survey (goblins ×2,213 north w/ ladder carts only siege equipment + ×899 east vs ~360 defenders); NPC perception gap (heroes visible/touchable, unheard/unseen — Jenna's porter test); linked quest confirmed (dam collapse flooded river → reinforcement cavalry blocked); strategy split: Edis holds north outer wall (~fifty-soldiers worth per party, bait doctrine sanctioned), Party 1 sorties east gate to stop dam collapse ("going after their heads"); Roderick Sajan's guerrilla-retreat proposal rejected. tower.md Defend row + Floor 10 section, heroes.md special-rules section + roster header Ch.26, glossary +7 terms, emotional_rules Two-Party Era extended (Edis dread→resolve, Jenna withheld-intel guilt, Aaron worry, Roderick absurdity/duty + matrix row), character file updates (han/jenna/aaron/eolka/roderick/anytng). Source normalizations: "Shuran"→Shurn (dominant form), "eat gates"→east gates ×2, "battling rams"→battering rams, garbled reinforcement sentence minimally reconstructed.

## Open Questions / Pending

- **Chapter 27**: not yet provided; Floor 10 battle underway — Party 1 sortied east toward the dam, Party 2 holding north wall vs ladder carts.
